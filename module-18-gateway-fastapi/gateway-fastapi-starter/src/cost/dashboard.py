...
1. **As part of the full gateway:** ``src.gateway.app`` mounts
   ``router`` on the main FastAPI app at startup, so the
...
2. **Standalone (this module's ``__main__``):** ``uv run python -m
   ...
   That's what ``make cost-dashboard`` uses so learners can render
   the dashboard without booting the full gateway.
...
# src.cost.dashboard``. The gateway mounts ``router`` on the full
# app instead, at which point this standalone path is just one
