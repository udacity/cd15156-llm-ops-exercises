...
in REQ-078) greps `content/implementation/` for forbidden bare
...
| `MODEL_COMPLEX` | `gpt-4o` | 28 | Matches capstone `project/src/pricing.py:20`. Locked into `src/pricing.py` cost-table when REQ-069 lands. |
| `MODEL_SIMPLE` | `gpt-4o-mini` | 29 | Matches capstone `project/src/pricing.py:21`. Used by the simple-vs-complex router (REQ-071/M18). |
| `EMBEDDING_MODEL` | `text-embedding-3-small` | 30 | Matches capstone. 1536-dim, cheap, sufficient quality for documentation retrieval. |
| `EMBEDDING_DIM` | `1536` | 31 | OpenAI default for `text-embedding-3-small`. Asserted at Chroma collection create time (M05). |
| `DEFAULT_TOP_K` | `5` | 34 | Matches capstone `project/src/rag/pipeline.py:9`. M11 (REQ-068) sweeps k∈{3,5,10} as a calibration exercise. |
...
| `CACHE_SIMILARITY_THRESHOLD` | `0.85` | 45 | Matches capstone semantic cache. Empirically tuned — see capstone REQ-018 history. |
...
| `CLIENT_ID_HEADER` | `X-Client-Id` | 55 | M22 (REQ-073) sticky-by-user A/B bucketing key. Cross-module contract — verified by `tests/test_smoke.py` after REQ-071. |
