...
**Setup.** The starter already ships `QueryResponse` in `src/models.py` — that model mirrors `project/src/models.py` and is referenced from a dozen places. Don't modify it. Instead, add a companion model `QueryResponseValidator(BaseModel)` next to it with the stricter constraints the planning doc calls for: `answer: str`, `sources: list[Source] = Field(..., min_length=1)`, `confidence: float = Field(..., ge=0.0, le=1.0)`. The planning doc names the citations field `citations` — at the boundary in our codebase the field is `sources` (same concept; the existing code uses `sources` and the validator matches the existing wire shape rather than renaming downstream callers).

**Wire at the gateway boundary.** In `src/gateway/routes.py:query_endpoint`, after the hallucination check passes and just before the final `return response`, wrap the response in `QueryResponseValidator.model_validate(response.model_dump())` inside a `try/except ValidationError` block. On the exception, return `JSONResponse(status_code=502, content={"detail": "output_validation_failed", "field": str(exc.errors()[0]['loc'][0])})`. Bracket the new block with `# TODO(m20-exercise-4)-start` and `# TODO(m20-exercise-4)-end` markers so the next learner can grep the seam. The 502 (Bad Gateway) status is deliberate: this is a contract failure on our side, not a client error, so a 4xx would mislead the operator triaging the alert.

**Write two tests.** Add `tests/test_gateway_output_validator.py` with two tests. Test 1 patches `src.gateway.routes.route_query` to return a well-formed `QueryResponse` (one source, `confidence=0.92`) and patches `src.gateway.routes.check_hallucination` to return `(True, None)`. The `POST /query` call should return 200 and the JSON body should round-trip through `QueryResponseValidator`. Test 2 patches `route_query` to return a `QueryResponse` with `sources=[]` and the same hallucination-pass patch. The call should return 502 with a body of `{"detail": "output_validation_failed", "field": "sources"}`. Reset the rate-limit bucket with `reset_rate_limit_state()` at the top of each test — `/query` runs the LLM10 check before reaching the validator and tests sharing a process can otherwise interfere.

**Acceptance.** `uv run pytest tests/test_gateway_output_validator.py -v` exits 0 with both tests green. The `grep -n "QueryResponseValidator" src/models.py src/gateway/routes.py` shows the model definition and one use at the boundary. The `grep -n "TODO(m20-exercise-4)" src/gateway/routes.py` shows the start and end markers bracketing the new block. Manual check: with `make serve` running, fire a `curl` against `/query` with a deliberately off-corpus question (e.g. `What is the chemical formula for water?`); if your `store.query` returns no sources for that input the validator will fire and you should see a 502 with the structured error body.

**What this exercise does not cover.** Validating the input — `QueryRequest` already pins `question` to 1-4000 chars and `top_k` to 1-20; the input side is solved. Validating against a JSON Schema or OpenAPI contract — Pydantic models cover the static shape, and FastAPI auto-generates the schema; production teams sometimes also enforce a frozen `openapi.json` in CI to catch unintended drift, but that is one layer up from per-request validation. Custom validators on individual fields — Pydantic supports `@field_validator` for cross-field checks (e.g. confidence must be 0.0 when `sources` is empty) that the basic constraint vocabulary cannot express. The exercise teaches the boundary discipline; the production engineering question is "what is the contract this service promises and where is it enforced," and this exercise answers it for the response surface.
