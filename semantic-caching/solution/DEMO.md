...
# Demo: Wire the Chroma Cache, Watch a Paraphrase Hit, Tune the Threshold

A semantic cache has four moves — embed the query, similarity-search the cache, return on hit, call the model and write back on miss — and one parameter that decides whether the whole thing earns its keep: the similarity threshold. This demo turns that into running code in the ScikitDocs starter. You will look at the cache module, fire a query that warms the cache, fire a paraphrase that hits it, then walk the threshold up and down and watch the hit-miss boundary move. The pipeline does not have an HTTP route yet, so this exercise composes the cache around `run_pipeline` directly in Python. The composition is identical to what an HTTP gateway would reproduce inline at the request boundary; doing it in Python first makes the layering legible.
...
The `cached_route_query` helper at `src/cache/wrapper.py:25-50` composes the cache around `run_pipeline` so this module's demo and exercises have a clean entry point without standing up an HTTP server. A production HTTP gateway would reproduce the same composition inline at the request boundary — input guards, then cache lookup on the cleaned text, then the tiered router on miss, then output guards, then the conditional cache store only after guards pass. The safe-default that comes out of that composition — never write a response from an input that tripped a guard — is what protects the cache from poisoning.
