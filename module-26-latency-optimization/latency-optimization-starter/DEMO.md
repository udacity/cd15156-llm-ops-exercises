...
Read the trace top to bottom. The `rag_query` trace is the pipeline root; under it sit the retrieval and generation spans. The two steps `route_query` runs before the pipeline — the cache lookup and, on a miss, the tier classifier — each make their own API call before that span opens, so Phoenix records them as separate short top-level traces next to `rag_query`, not as children of it. Read them together to reconstruct the full `route_query` budget. Treat the magnitudes below as workload- and region-dependent — the shape is the point, not the numbers.

On a cache miss, the tier classifier runs. The two-label JSON-mode prompt at `prompts/classifier.j2` goes through `gpt-4o-mini` and returns either `"simple"` or `"complex"`. On hosted endpoints this is typically in the few-hundred-millisecond range. The gateway module covered the classifier internals; the trace surfaces it as its own short OpenInference trace emitted by the `OpenAIInstrumentor` that the tracing module wired into `init_tracing`.

The cache lookup actually comes first in `route_query`: it runs before the classifier, so a hit returns without any LLM call at all. The lookup itself is one embedding call to `text-embedding-3-small` plus a Chroma nearest-neighbor query against the `cache` collection. On a hosted endpoint the embedding round-trip dominates, so the lookup is a few hundred milliseconds to about a second, not single-digit milliseconds. On a hit, the classifier and the entire downstream pipeline are skipped and the response returns in roughly that lookup time, several-fold faster than the cold path. The semantic-caching modules anchored the cache; the threshold gate is `src.constants.CACHE_SIMILARITY_THRESHOLD = 0.85`.

The retrieval span comes from `traced_pipeline` at `src/tracing.py`. One embedding call for the question, one Chroma similarity search against the `scikit_docs` collection. At the starter's ~750 chunks, that lands in the single-digit-millisecond range. The vector-databases module covered the HNSW algorithm; here it is a small constant.
...
Output guards do not run on the blocking path the starter ships today. A hallucination judge and an off-topic check that fire after generation are the guardrails surface — those scanners run against the same gateway and add latency you can measure. The layered ML input guards from the guardrails module are off here too, via `ENABLE_ML_INPUT_GUARDS=false` in `.env`: DeBERTa plus Presidio run model inference on every blocking request, about 40 ms on the workspace GPU and about 900 ms on CPU, and that cost would sit on top of every span you are about to read. For this demo, the blocking `/query` route runs cache lookup → classifier → retrieve → generate and returns. That is the surface Exercise 1 profiles.
...
The honest framing. At the starter's ~750 chunks the curve is barely measurable: the ordering reproduces (higher `ef_search` is slower) but the values sit within a fraction of a millisecond of each other across the typical sweep range. That flatness is the lesson, not a defect; at a few hundred chunks the curve runs along the noise floor. At a few million the tuning becomes operationally meaningful and the recall-versus-latency trade becomes a real engineering decision. The discipline transfers regardless of scale: profile before you tune, and tune the knob whose curve sits above the noise floor on your specific workload. The exercise that follows lets you run a sweep on the starter's corpus and see for yourself.
