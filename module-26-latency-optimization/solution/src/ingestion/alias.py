...
Environments where no migration has run have no ``ACTIVE_COLLECTION``
file; ``read_active_collection`` returns the literal ``scikit_docs`` so
the original behaviour is preserved for any caller that imports
