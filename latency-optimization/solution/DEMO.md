...
Output guards do not run on the blocking path the starter ships today. A hallucination judge and an off-topic check that fire after generation are the guardrails surface — those scanners run against the same gateway and add latency you can measure. For this demo, the blocking `/query` route runs cache lookup → classifier → retrieve → generate and returns. That is the surface Exercise 1 profiles.
