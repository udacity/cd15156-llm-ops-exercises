...
The starter adds a streaming variant of `/query` — `POST /query/stream`,
mounted from `src/streaming.py` — to demonstrate time-to-first-token
improvements. The non-streaming `/query` route stays unchanged so the
evaluation harness and other consumers aren't broken.
