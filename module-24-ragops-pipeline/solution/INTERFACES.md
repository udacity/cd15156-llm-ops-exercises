...
The default collection name `scikit_docs` is a *public alias*. The
blue/green index swap creates two real collections (`scikit_docs_blue`,
`scikit_docs_green`) and records which one is active in
`data/ACTIVE_COLLECTION`; `get_collection("scikit_docs")` resolves the
alias to the active color. Passing an explicit color name bypasses the
alias.
