...
Quality parity does not settle the question: two variants that score statistically tied on quality can still have meaningfully different cost or latency profiles, and either dimension can decide the winner. The analyzer already computes both; this exercise reads them and forces a written decision.
...
- **Pointing the script at `gpt-4o` instead of `gpt-4o-mini`.** Costs go up roughly tenfold on the same workload. The asymmetry warning at the end of Exercise 1 is the canonical example of why cost asymmetry is the first LLM-specific A/B pitfall.
