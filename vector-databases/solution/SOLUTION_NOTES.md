...
- **`sentence-transformers` is not in `pyproject.toml`.** Exercise 3 step 2 instructs the learner to run `uv add sentence-transformers`, which mutates both `pyproject.toml` and `uv.lock`. Pre-adding the dependency here would require regenerating `uv.lock`, which the initial scaffolding explicitly prohibits (no `uv sync` during build verification). The learner is expected to run `uv add sentence-transformers` themselves before executing `scripts/embed_with_st.py`, `scripts/load_data_st.py`, or `scripts/recall_at_5_st.py`.
- **Reference recall numbers (10/12, ~90s MiniLM rebuild) are from the spec, not a re-measurement.** Verifying them would require `make load-data` (OpenAI billing) plus a cold MiniLM rebuild, both excluded from the build-time verification per the initial scaffolding's cost guard.
