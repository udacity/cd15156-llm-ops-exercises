...
```bash
code exercises/05-vector-databases/vector-databases-starter/DEMO.md exercises/05-vector-databases/vector-databases-starter/INSTRUCTIONS.md exercises/05-vector-databases/vector-databases-starter/INTERFACES.md exercises/05-vector-databases/vector-databases-starter/src/chunker.py exercises/05-vector-databases/vector-databases-starter/src/embedder.py exercises/05-vector-databases/vector-databases-starter/src/store.py exercises/05-vector-databases/vector-databases-starter/src/models.py
```
...
Here's what to watch out for. If a build you expected to be cold finishes in under a second, you didn't really rebuild; you're looking at a stale collection. Delete the `data/chroma` directory and run the load again. You're done when the collection count matches what the load printed.
...
One practical note: run the script with the `PYTHONPATH=.` prefix, or the imports fail. You're done when it reports recall at five of at least zero point seven.
